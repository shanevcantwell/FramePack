    ...
    with block:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                components['input_image_gallery_ui'] = gr.Gallery(type="pil", label="Input Image(s)", height=320)
                components['prompt_ui'] = gr.Textbox(label="Prompt", lines=3)
                components['n_prompt_ui'] = gr.Textbox(label="Negative Prompt", lines=2)
                with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                components['last_finished_video_ui'] = gr.Video(interactive=True, autoplay=False)
