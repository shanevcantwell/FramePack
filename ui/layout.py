...
def create_ui():
    ...
    # Removed the custom CSS for button layout as it's no longer needed
    css = """
    ...
    with block:
        ...
        # TOP ZONE (TASK DEFINITION): Image, Prompts, and Add to Queue button
        with gr.Row():
            # Left column for image and its primary action
            with gr.Column(scale=1, min_width=300):
                components['input_image_gallery_ui'] = gr.Gallery(type="pil", label="Input Image(s)", height=420)
            ...
            # Right column for creative text inputs
            with gr.Column(scale=2, min_width=600):
                components['prompt_ui'] = gr.Textbox(label="Prompt", lines=15)
                components['n_prompt_ui'] = gr.Textbox(label="Negative Prompt", lines=6)

        # MIDDLE ZONE (QUEUE MANAGEMENT): All controls for the queue itself
        with gr.Group():
            # Process and Abort buttons are now part of the queue group
            with gr.Row():
        ...
        # BOTTOM ZONE: Settings and Output
