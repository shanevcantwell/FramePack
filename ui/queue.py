    ...
    while queue_state["queue"] and not shared_state.abort_event.is_set():
        ...
        while True:
            ...
            if flag == 'progress':
                ...
                if msg_task_id == current_task_id: yield (state_dict_gr_state, update_queue_df_display(queue_state), gr.update(value=last_known_output_filename), gr.update(value=preview_np_array), desc_str, html_str, gr.update(interactive=False), gr.update(interactive=True), gr.update(interactive=True))
            elif flag == 'file':
