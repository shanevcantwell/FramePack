...
def load_image_from_path(image_path):
    """
    Loads an image from a given path and returns updates for all image-related UI components.
    """
    pil_image = None
    if image_path and os.path.exists(image_path):
        try:
            ...
            pil_image = None # Ensure it's None on error

    has_image = pil_image is not None
    return (
        gr.update(value=pil_image, visible=has_image),   # INPUT_IMAGE_DISPLAY_UI
        gr.update(visible=has_image),                    # CLEAR_IMAGE_BUTTON_UI
        gr.update(visible=has_image),                    # DOWNLOAD_IMAGE_BUTTON_UI
        gr.update(visible=not has_image)                 # IMAGE_FILE_INPUT_UI
    )