...
def wire_events(components: dict):
    ...
    full_workspace_ui_components = [components[K[key]] for key in default_keys_map.keys()]
    ...
    add_task_outputs = (
        [components[k] for k in [K.APP_STATE, K.QUEUE_DF_DISPLAY, K.INPUT_IMAGE_DISPLAY, K.IMAGE_FILE_INPUT]] +
    ...
    process_q_outputs = [
        components[K.APP_STATE], components[K.QUEUE_DF_DISPLAY], components[K.LAST_FINISHED_VIDEO],
    ...
    (components[K.ADD_TASK_BUTTON].click(
        ...
        fn=event_handlers.update_button_states, inputs=[components[K.APP_STATE], components[K.INPUT_IMAGE_DISPLAY], components[K.QUEUE_DF_DISPLAY]],
    ...
    (components[K.CLEAR_QUEUE_BUTTON].click(
        fn=queue_actions.clear_task_queue_action, inputs=None, outputs=[components[K.APP_STATE], components[K.QUEUE_DF_DISPLAY]]
    ).then(
        fn=event_handlers.update_button_states, inputs=[components[K.APP_STATE], components[K.INPUT_IMAGE_DISPLAY], components[K.QUEUE_DF_DISPLAY]],
        outputs=button_state_outputs
    ))
    ...
    (components[K.LOAD_QUEUE_BUTTON].upload(
        fn=queue_actions.load_queue_from_zip, inputs=[components[K.LOAD_QUEUE_BUTTON]], outputs=[components[K.APP_STATE], components[K.QUEUE_DF_DISPLAY]]
    ).then(
        fn=event_handlers.update_button_states, inputs=[components[K.APP_STATE], components[K.INPUT_IMAGE_DISPLAY], components[K.QUEUE_DF_DISPLAY]], outputs=button_state_outputs
    ))

    (components[K.QUEUE_DF_DISPLAY].select(
